...
export async function run(command?: string) {
  if (command) {
    command = command.replace(/\{path\}/g, "tests/integration/file_manager_with_everything/expected_outputs/");
  }
  ...
  const fullCommand = `deno run --allow-read --allow-write tests/integration/file_manager_with_everything/cli.ts${command}`;
...
export function assertOutput(actual: string, expectedFile: string) {
  assertOutputHelper(
    ...
    expectedFile,
    false,
    "tests/integration/file_manager_with_everything/expected_outputs/" + expectedFile,
  );
}
...
Deno.test("should show file exists: read -D file_to_read.txt", async () => {
  const stdout = await run("read -D tests/integration/file_manager_with_everything/file_to_read.txt");
  asserts.assertEquals(stdout.trim(), "File exists.");
});

Deno.test("should show contents: read file_to_read.txt", async () => {
  const stdout = await run("read tests/integration/file_manager_with_everything/file_to_read.txt");
...
Deno.test("should show contents with extra log value: read -L file_to_read.txt", async () => {
  const stdout = await run("read -L SomeValue tests/integration/file_manager_with_everything/file_to_read.txt");
...
Deno.test("should do a dry run and add an extra log value: read -L --dry-run file_to_read.txt", async () => {
  const stdout = await run("read -L SomeValue --dry-run tests/integration/file_manager_with_everything/file_to_read.txt");
  asserts.assertEquals(stdout, "SomeValue\nFile exists.");
});
